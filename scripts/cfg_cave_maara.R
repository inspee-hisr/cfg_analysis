#!/usr/bin/env Rscript
# Cave-specific analysis for Spilaio Maara (Cave ID 118).
# Loads results/cfg_data_long.tsv directly — no spatial dependencies.
# Outputs: results/maara_*.tsv  |  plots/maara_*.png
...
maara <- data_long |>
    filter(CaveName == "Spilaio Maara")

cat(sprintf("Spilaio Maara: %d rows (species × reference combinations)\n",
            nrow(maara)))
...
n_species      <- n_distinct(maara$Species)
n_endemic      <- maara_sp |> filter(Distribution == "Endemic to Greece") |> nrow()
n_cave_endemic <- maara_sp |>
    filter(Locus_Typicus_Cave == "Spilaio Maara") |> nrow()
n_troglo       <- maara_sp |> filter(Classification == "Troglobiont")  |> nrow()
n_stygo        <- maara_sp |> filter(Classification == "Stygobiont")   |> nrow()
...
species_summary <- tibble(
    metric = c(
        "Total species",
        "Endemic to Greece",
        "Cave-endemic (described from Maara)",
        "Troglobionts",
        "Stygobionts",
...
save_tsv(species_summary, "maara_species_summary")
...
print(clf_counts)
save_tsv(clf_counts, "maara_classification")
...
p_clf <- ggplot(clf_counts, aes(x = Classification, y = n_species,
    ...
    coord_flip() +
    labs(title    = "Spilaio Maara — species by ecological classification",
         subtitle = sprintf("%d species total", n_species),
         x = NULL, y = "Number of species") +
    theme_cfg_bar()
save_plot(p_clf, "maara_classification", w = 18, h = 10)

# ── species by taxonomic order ────────────────────────────────────────────────
order_counts <- maara_sp |>
    count(Order, name = "n_species") |>
    mutate(Order = fct_reorder(Order, n_species))

cat("\n--- Species by Order ---\n")
print(order_counts)
save_tsv(order_counts, "maara_orders")
...
p_ord <- ggplot(order_counts, aes(x = Order, y = n_species)) +
    ...
    scale_y_continuous(expand = expansion(mult = c(0, 0.25))) +
    coord_flip() +
    labs(title    = "Spilaio Maara — species by taxonomic order",
         subtitle = sprintf("%d orders represented", nrow(order_counts)),
         x = NULL, y = "Number of species") +
    theme_cfg_bar()
save_plot(p_ord, "maara_orders", w = 18, h = 12)

# ── conservation status ───────────────────────────────────────────────────────
iucn_counts <- maara_sp |>
...
print(iucn_counts)
save_tsv(iucn_counts, "maara_iucn")
...
p_iucn <- ggplot(iucn_counts, aes(x = IUCN_short, y = n_species,
    ...
    labs(title    = "Spilaio Maara — IUCN Red List status of species",
    ...
    theme_cfg()
save_plot(p_iucn, "maara_iucn", w = 16, h = 10)
...
print(ref_summary)
save_tsv(ref_summary, "maara_ref_summary")
...
save_tsv(ref_species, "maara_ref_species")
...
p_refs <- ggplot(ref_species, aes(x = label, y = n_species)) +
    ...
    scale_y_continuous(expand = expansion(mult = c(0, 0.25))) +
    coord_flip() +
    labs(title    = "Spilaio Maara — species documented per reference",
         subtitle = sprintf("%d references total", n_refs),
         x = NULL, y = "Number of species") +
    theme_cfg_bar(base = 10)
save_plot(p_refs, "maara_ref_species", w = 20, h = 14)
...
print(accum)
save_tsv(accum, "maara_species_accumulation")
...
p_accum_clf <- ggplot(accum_clf,
    ...
    scale_x_continuous(breaks = pretty_breaks(n = 10)) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.12))) +
    labs(title    = "Spilaio Maara — cumulative species by classification",
         subtitle = "Each line = running total for one ecological category",
         x = "Year of first record", y = "Cumulative species",
         colour = "Classification") +
    theme_cfg()
save_plot(p_accum_clf, "maara_accumulation_by_classification", w = 20, h = 12)
...
cave_endemics <- maara_sp |>
    filter(Locus_Typicus_Cave == "Spilaio Maara") |>
    select(Species, Order, Class, Classification, Distribution,
           IUCN_Red_List, Greek_Red_Data_Book)

cat("\n--- Species described from Spilaio Maara ---\n")
print(cave_endemics)
save_tsv(cave_endemics, "maara_cave_endemics")

cat("\n================================================================\n")
cat("DONE — outputs written to results/maara_*.tsv and plots/maara_*.png\n")
cat("================================================================\n")
